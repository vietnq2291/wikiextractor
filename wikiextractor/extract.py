...
def compact(text, mark_headers=False):
    ...
    for line in text.split('\n'):
        if not line:
            ...
            while listLevel:
                # page.append(listClose[listLevel[-1]])
    ...
    ### END NEW CODE -----------------------------
        ...
        # Handle lists
            ...
            # Close lists that are no longer nested
            while listLevel and line[0] != listLevel[-1]:
                # page.append(listClose[listLevel[-1]])
                listLevel = listLevel[:-1]

            # Open new list if it's a different type
            if not listLevel or line[0] != listLevel[-1]:
                # page.append(listOpen[line[0]])
                listLevel += line[0]
...
class Extractor():
    ...
    def extract(self, out, html_safe=True):
        ...
        # Process each section to extract the header and content
        full_text = "\n".join(text)
        ...
        for header, content in processed_sections:
            if self.to_json:
                ...
                header = '<doc id="%s" url="%s" title="%s">\n%s\n\n' % (str(uuid.uuid4()), self.url, self.title, header)
                footer = "\n</doc>\n"
                out.write(header)
                out.write(content)
                out.write('\n')
